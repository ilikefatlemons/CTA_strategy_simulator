...
def test_无未来函数_截断法(prep, res, 切):
    ...
    **投毒仍然打在 1m 帧上**(那是数据源), 但比较要在 **15m** 下标空间做 ——
    ...
    切15 = int(脏tf["15m"].bin_of[i])       # 第一根被弄脏的 15m

    def 之前(交易):
        return [(t.方向, t.入场下标, t.入场价, t.出场下标, t.出场价,
                 t.理由, t.仓位比例, t.跳空成交)
                for t in 交易 if t.出场下标 < 切15]
...
def test_截断法本身不是空真(prep, res):
    ...
    脏tf = build_timeframes(脏, tfs=计算周期)
    got = 跑回测(dataclasses.replace(prep, df=脏), 脏tf)
    切15 = int(脏tf["15m"].bin_of[i])
    后 = lambda r: [t.出场价 for t in r.交易 if t.出场下标 >= 切15]
    assert 后(got) != 后(res)


# ------------------------------------------------ 这一版特有的两条不变量 ----
def test_入场成交价就是那根15m的open(res, tf):
    """
    15m 是**唯一驱动时钟**, 所以这条从「基本成立、有闸门例外」变成了**恒等式**:
    `一笔.入场下标` 就是 15m 的下标, 成交价就是那根的 `open`, 一笔例外都不许有。

    旧版 1m 驱动时这里允许一类例外 (入场被时段首根闸门推迟到 15m 内部的某一分钟)。
    换成 15m 驱动之后闸门本身就作用在 15m 上, 那类例外结构上不可能再出现。
    """
    o15 = tf["15m"].bars["open"].to_numpy("float64")
    assert res.交易, "一笔都没有"
    坏 = [t for t in res.交易 if abs(t.入场价 - o15[t.入场下标]) > 1e-12]
    assert not 坏, f"{len(坏)}/{len(res.交易)} 笔的入场价不等于那根 15m 的 open"


def test_引擎不读1m价格(res, tf, prep):
    """
    所有出场价要么落在 15m 的 [low, high] 内, 要么等于 15m 的 open (跳空补价)。
    若引擎偷偷用了 1m 的极值, 出场价就可能落在 15m bar 的范围之外。
    """
    b = tf["15m"].bars
    ...
    for t in res.交易:
        ...
        assert lo[j] - 1e-9 <= t.出场价 <= hi[j] + 1e-9 or abs(t.出场价 - op[j]) < 1e-12, (
            f"出场价 {t.出场价} 落在 15m bar [{lo[j]}, {hi[j]}] 之外 @{t.出场时间}")
...
def test_三条止损线满足取大取小的恒等式(res):
    """
    引擎记三条逐 15m 的线, 恒等式是 `止损线 = max(固定止损线, 吊灯原线)` (空头取 min)。
    ...
    # 方向以 `res.仓位` 为准, 但它盖不住**出场那一根**: 出场块先写 `止损线[i]` 再把
    # 批置空, 而 `仓位[i]` 是随后的 MTM 块写的, 于是出场根上「有线但仓位为 0」。
    # 这两处的差集必须恰好等于出场根的集合 —— 顺带把这条不对称本身钉死。
    有 = ~np.isnan(有效)
    出场根 = np.zeros(len(有效), dtype=bool)
    for t in res.交易:
        出场根[t.出场下标] = True
    assert np.array_equal(有, (res.仓位 != 0) | 出场根), (
        "止损线的有值区间 ≠ 持仓根 ∪ 出场根")
...
def _独立重算出场(t, res, tf15, 含本根: bool):
    """
...
def test_吊灯的极值严格滞后一根_不含本根的high_low(res, tf):
    tf15 = tf["15m"]
    """
    **截断法结构上抓不到这条。** 截断法把 bar i 之后的数据改成垃圾, 验的是「有没有读
    ...
    的出场。换成 15m 驱动之后这个风险更大: 一根 15m 的振幅比一根 1m 大得多。
    """
    ...
    # 下面那个空真守卫会证明它确实分得开。
    坏 = [t for t in res.交易
          if not 同(_独立重算出场(t, res, tf15, 含本根=False), t, 1e-9)]
    assert not 坏, (
    ...
    分歧 = [t for t in res.交易
            if not 同(_独立重算出场(t, res, tf15, 含本根=True), t, 1e-9)]
    assert 分歧, "含本根版与引擎完全一致 —— 这条测试是空真的, 夹具里没有能分辨的 bar"
    差 = [abs(_独立重算出场(t, res, tf15, 含本根=True)[1] - t.出场价) for t in 分歧
          if _独立重算出场(t, res, tf15, 含本根=True) is not None]
