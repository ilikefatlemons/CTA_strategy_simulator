...
**两页各四格**, 每格三层 (主图 + ATR + MACD)：

    页1 决策层   15m  30m        页2 执行层   1m   5m
                 2h   1d                      15m  30m

左上角品种下拉, 下面一行是 `[决策层][执行层] ｜ [MA][ATR][MACD]`; 右上角统计面板与
五个标注开关。**默认只显示 Entry 与出场箭头**, 其余全关。

策略规格见 `docs/02-lineA-多周期回调/A3-单个自洽策略实施+改进/20260824-骨架+前端phase/goal.md`:
