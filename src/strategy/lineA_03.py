...
class 策略开关:
    ...
    **只有一个真开关。**
    ...
    # 吊灯用哪个 ATR。**固定止损始终用入场冻结的 ATR₀, 不受这个开关影响。**
    #
    #   True  (二稿 L56) 每根重取「截至本根、最后一根已收盘 30m」的 ATR(14)
    #   False            吊灯也用入场冻结的 ATR₀
    #
    # 随动的代价: 追踪线**不再单调** —— ATR 上升而极值不变时它会后退。这是刻意接受
    # 的, 不是疏漏 (2026-08-28 决定)。
    止盈_ATR随动: bool = True

    # 止盈阀门 (二稿 L57「吊灯需要加一个阈值设计, 不能 TP 还亏损」)。
    #
    # 开着时: 本根算出来的吊灯线若**差于入场价** (多头低于 / 空头高于), 这一根就不
    # 许用吊灯出场, 有效止损退回固定止损。**逐根判, 不是闩锁** —— 线抬上去了就能用,
    # 掉下来了又不能用。
    #
    # 为什么需要它: 记 R = 1.5·ATR₀, 则固定止损 = 入场 −1R、吊灯 = 极值 −2R,
    # 两条线在 MFE=+1R 处接上、在 MFE=+2R 处才回到入场价。所以 MFE 落在 +1R~+2R 之间
    # 时吊灯已生效却坐在入场价下方 —— 那一段被它打掉的交易**结构性地是亏损单**。
    #
    # 代价不是白赚: 这批单子原本被砍在 (−1R, 0) 之间, 阀门挡住后它们会一路跑到 −1R
    # 或者反弹回来。净效果必须实测。
    止盈_不得差于入场价: bool = True
