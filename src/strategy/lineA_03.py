...
class 策略开关:
    ...
    **只有一个真开关。**
    ...
    #
    # C1 `冷静期_解除用状态跨越`
    #   开: R2′ —— 2h 的**不带粘性**三值态发生跨越就解除
    #       解除 <=> state(i-1) ∈ {多,空} 且 state(i-2) != state(i-1)
    #   关: 现行 —— 连续 `冷静期_解冻根数` 根同方向的干净排列才解除
    #
    #   注意 R2′ **不是**单调更快: AU #3 那次冻结只有 3 根 2h, 而首次满足 R2′ 要等
    #   16 根 —— 现行规则反而先解除。R2′ 换的是判据不是快慢。
    冷静期_解除用状态跨越: bool = True

    # C2 `冷静期_信号层停摆`
    #   开: 冻结期间信号层完全停摆 —— 进入时清回调武装, 期间不武装、不求开关、
    #       不入场; 解除后必须重走一次完整 setup (新的 15m 反向 + 新的水下金叉)。
    #   关: 现行 —— 冻结期间信号层照常累积, 只是最后被闸门挡住。
    #
    #   动机不是「闩锁会变旧」(那条已被数据证伪), 而是**冷静期没有真的冷静**:
    #   AU 上 3/10 的冻结在解除那一刻条件已齐, 而那个条件是在本该停摆的期间攒的。
    冷静期_信号层停摆: bool = True
...
def 冷静期该解除(前态: int, 今态: int) -> bool:
    """
    R2′ —— 冷静期解除判据。**两个入参必须是不带粘性的三值态**(`三线状态` 的输出)。
    ...
    真值表:
        ...
        前态      今态      解除
        未定      多/空      ✓      从混乱走到干净
        空        多         ✓      直接反转也算 —— 恢复的是没在亏的那个方向
        多        空         ✓
        多        多         ✗      一直干净不算「跨越」
        空        空         ✗
        任意      未定       ✗      今态必须干净

    **为什么必须不带粘性**: 粘性方向会把「未定」前向填充成上一个方向, 于是
    `未定 -> 干净同向` 这一跨越在粘性口径下比出来是「相同」, 判据直接失效。
    `tests/test_lineA_03_strategy.py::test_冷静期解除必须用不带粘性的态` 钉这一条。

    调用方负责 K1(只在冻结起点之后收盘的那根上评估), 这个函数本身无状态、幂等。
    """
    return bool(今态 != 未定 and 今态 != 前态)
...
def 动量开关段(dif: np.ndarray, dea: np.ndarray, 最少根数: int = 35):
    """
    ...
    **不设段龄上限。** 理论上一次水下金叉能授权此后整段行情里的任何时刻入场, 但
    ...
    if n == 0:
        return 多开, 空开, np.zeros(0, dtype=np.int64)

    段起点 = np.full(n, -1, dtype=np.int64)
    多闩 = False          # 多头闩锁: 水下金叉开, 下一次死叉关
    空闩 = False          # 空头闩锁: 水上死叉开, 下一次金叉关
    起 = -1               # 当前这一段是被哪一根上的交叉打开的
    for b in range(n):
        ...
        多开[b] = 多闩
        空开[b] = 空闩
        段起点[b] = 起 if (多闩 or 空闩) else -1
    return 多开, 空开, 段起点


def 动量开关(dif: np.ndarray, dea: np.ndarray, 最少根数: int = 35):
    """`动量开关段` 丢掉段起点。多数调用方只要那两个 bool 数组。"""
    多开, 空开, _ = 动量开关段(dif, dea, 最少根数)
    return 多开, 空开
