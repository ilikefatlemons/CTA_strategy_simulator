...
# 调参区
# ---------------------------------------------------------------------------
START = "2025-07-01"      # 约 250 个交易日。窗口越长, 首次选中一个品种越慢:
END = "2026-07-29"        # 瓶颈不是回测(130 万根 0.34s), 是往图表推数据
DEFAULT_SYMBOL = "RB"     # (js_data 实测 8.5 万行 0.36s / 16MB, 100 万行 3.7s / 193MB)
...
def main() -> None:
    ...
    print(f"{len(symbols)} symbols | window {START} ~ {END} | 时段 {SESSION_MODE}")
