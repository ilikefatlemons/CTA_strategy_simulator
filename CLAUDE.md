...
**驱动时钟是 15m**（2026-08-26 从 1m 改过来）。引擎一个 1m 价格都不读：入场成交在
15m 的 open，出场对 15m 的 low/high 测，吊灯极值只累计到 **15m 的 i-1**。30m 只出一个
冻结的 ATR₀，2h 只出方向与冷静期，**5m 与 1m/1d 完全不参与计算**，只是屏幕上的格子。

`一笔.入场下标 / 出场下标`、`仓位 / 止损线 / 固定止损线 / 吊灯原线 / 权益`、四类观测点的
`下标`——**全是 15m 下标**，长度 = `tf["15m"].n_bars`。图层要落到别的格上，自己经
`chart_lineA_03.驱动到1m` 换算（入场与状态点用首、出场用末）。

理由：1m 驱动等于假装知道 15m bar 的内部走势。这一版的立场是**不知道**——每个周期只用
它自己那条 bar 序列。代价是止损每 15 分钟才检查一次、成交更差、笔数更少，这是刻意的。
`回.closed_pos[首] == i-1` 在引擎启动时被断言，那是「只用 bar i-1 及更早」的机械凭据。
