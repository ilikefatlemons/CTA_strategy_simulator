...
- 抓取后落地为本地 CSV（`data/00-美股ETF历史/raw/{symbol}_5m.csv`），下游所有层只读 CSV，不直接依赖 alpaca——方便以后换数据源或做校准
...
2. **Phase 1**：alpaca 数据抓取（NVDA/KO/XOM/JPM, 5m bar, 60天）→ 落地 CSV，确认 schema ✅ 已完成 (`src/data/fetch_alpaca.py` → `data/00-美股ETF历史/raw/{symbol}_5m.csv`, 各4680行)
