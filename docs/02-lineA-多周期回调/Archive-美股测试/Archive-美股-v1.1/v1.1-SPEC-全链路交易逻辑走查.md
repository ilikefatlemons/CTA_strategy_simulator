...
- 数据源头：`fetch_alpaca.py`的`main()`（第85-92行）逐个symbol调用`fetch_symbol(client, symbol, sector)`（第0节实现），写到`data/00-美股ETF历史/raw/{symbol}_5m.csv`——`run_phaseF.py`第26行读的正是这个文件，形成"抓取脚本产出 → 回测脚本消费"的完整闭环。
